...
# ================= build-30: writable kernel-cache location (the final cache fix) =================
# build-29 on-device diagnostics proved: Metal archive serialization WORKS on iOS
# (tmp_serialize_ok=1); the failure was NSCocoaErrorDomain "no permission" creating the cache
# directory, because path_cache_get resolves to $HOME/.cache = the app container ROOT, which
# iOS forbids writing to. tmp/ and Library/Caches are the writable locations. Redirect the
# Cycles kernel cache to Library/Caches so the tmp-serialize + move lands, kernels persist,
# and relaunches load near-instantly.
KM = "blender/intern/cycles/device/metal/kernel.mm"
edit(KM, [
...
''',
   '''    metalbin_path = path_cache_get(path_join("kernels", metalbin_name));
#  ifdef WITH_APPLE_CROSSPLATFORM
    /* build-30: path_cache_get resolves to $HOME/.cache = the app container ROOT, which iOS
     * forbids writing to - the true cause of every archive save failure (Metal's "Invalid
     * URL" just meant the destination directory could never exist). Library/Caches is the
     * sanctioned writable cache location. */
    {
      NSString *ios_caches = [NSSearchPathForDirectoriesInDomains(
          NSCachesDirectory, NSUserDomainMask, YES) firstObject];
      metalbin_path = path_join(string([ios_caches UTF8String]),
                                path_join("cycles-kernels", metalbin_name));
    }
#  endif
    path_create_directories(metalbin_path);
''',
...
print("BUILD-30 (writable kernel cache in Library/Caches) APPLIED OK")


# ================= build-31: compile-time attack + kernel export/seed via Files app =================
# "Stuck at 13" = integrator_shade_volume: the volumetric kernels are the heaviest to compile
# after Blender 5.0's volume changes (official upstream regression #150119) and were observed
# failing with XPC kills + silent retries on-device. The default scene doesn't even use
# volumes (kernel_features had no KERNEL_FEATURE_VOLUME bit).
# 1. Skip volumetric kernels when the scene has no volumes (same pattern upstream uses for
#    raytrace/MNEE kernels). Loading a volume scene compiles them then.
# 2. Disable MetalRT on iPad: hardware-RT kernels are dramatically more expensive to compile;
#    software BVH2 renders fine and the compile saving is worth far more here.
# 3. Export every compiled kernel archive to Documents/cycles-kernels (visible in the Files
#    app thanks to UIFileSharingEnabled) so users can hand them over for shipping in the IPA.
# 4. Seed the kernel cache from cycles-kernels-seed/ bundled inside the IPA (and from files
#    dropped into Documents/cycles-kernels), keyed by the same content hash - mismatches are
#    simply ignored and compile locally as before.
...
#  ifdef WITH_APPLE_CROSSPLATFORM
  /* build-31: the volumetric kernels are by far the heaviest to compile since Blender 5.0's
   * volume changes and were observed taking 300+ s and getting the Metal compiler service
   * killed on iPad. Skip them for scenes without volumes (same pattern as raytrace/MNEE
   * above); they compile on demand when a volume scene is loaded. */
...
#  endif
''',
   'b31-skip-volume-kernels'),
  ('''    loading_existing_archive = path_cache_kernel_exists_and_mark_used(metalbin_path);
''',
   '''#  ifdef WITH_APPLE_CROSSPLATFORM
    /* build-31: seed the kernel cache from (a) archives bundled inside the IPA and (b)
     * archives dropped into Documents/cycles-kernels via the Files app. Keyed by content
     * hash: mismatching OS/GPU/source simply misses and compiles locally as before. */
    if (!path_exists(metalbin_path)) {
      NSString *ios_rel = @(metalbin_name.c_str());
      NSString *ios_docs_dir = [NSSearchPathForDirectoriesInDomains(
          NSDocumentDirectory, NSUserDomainMask, YES) firstObject];
      NSArray<NSString *> *ios_seed_candidates = @[
        [[[NSBundle mainBundle] resourcePath]
            stringByAppendingPathComponent:[@"cycles-kernels-seed"
                                               stringByAppendingPathComponent:ios_rel]],
        [ios_docs_dir stringByAppendingPathComponent:[@"cycles-kernels"
                                                         stringByAppendingPathComponent:ios_rel]],
      ];
      NSFileManager *ios_seed_fm = [NSFileManager defaultManager];
      for (NSString *ios_cand in ios_seed_candidates) {
        if ([ios_seed_fm fileExistsAtPath:ios_cand]) {
          [ios_seed_fm copyItemAtPath:ios_cand toPath:@(metalbin_path.c_str()) error:nil];
          metal_printf("build-31 seed: using %s", [ios_cand UTF8String]);
          break;
        }
      }
    }
#  endif
    loading_existing_archive = path_cache_kernel_exists_and_mark_used(metalbin_path);
''',
   'b31-seed-from-bundle'),
  ('''            path_cache_kernel_mark_added_and_clear_old(metalbin_path);
            metal_printf("build-26 archive: saved %s", metalbin_path.c_str());
''',
   '''            path_cache_kernel_mark_added_and_clear_old(metalbin_path);
            metal_printf("build-26 archive: saved %s", metalbin_path.c_str());
            /* build-31: mirror into Documents/cycles-kernels so compiled kernels are visible
             * in the Files app and can be shipped inside future IPAs as a seed. */
            {
              NSString *ios_docs_dir = [NSSearchPathForDirectoriesInDomains(
                  NSDocumentDirectory, NSUserDomainMask, YES) firstObject];
              NSString *ios_export_path = [ios_docs_dir
                  stringByAppendingPathComponent:[@"cycles-kernels"
                                                     stringByAppendingPathComponent:
                                                         @(metalbin_name.c_str())]];
              [ios_fm createDirectoryAtPath:[ios_export_path stringByDeletingLastPathComponent]
                  withIntermediateDirectories:YES
                                   attributes:nil
                                        error:nil];
              [ios_fm removeItemAtPath:ios_export_path error:nil];
              [ios_fm copyItemAtPath:ios_bin_path toPath:ios_export_path error:nil];
            }
''',
   'b31-export-to-documents'),
])
...
edit(DI, [
  ('    use_metalrt = info.use_hardware_raytracing;\n',
   '''    use_metalrt = info.use_hardware_raytracing;
#  ifdef WITH_APPLE_CROSSPLATFORM
    /* build-31: hardware ray tracing makes the intersect/shade kernels dramatically more
     * expensive to compile; on iPad the one-time compile cost far outweighs the render
     * speedup. Software BVH2 traversal instead (CYCLES_METALRT=1 env still re-enables). */
    use_metalrt = false;
#  endif
''',
   'b31-no-metalrt'),
...
print("BUILD-31 (skip volume kernels + no MetalRT + kernel export/seed) APPLIED OK")
