...
# ============ build-18: magic-keyboard save-as + cursor offset + finger pinch + editor routing + window back button ============
...
edit('blender/intern/ghost/intern/GHOST_WindowIOS.mm', [
  ...
  ('- (void)endFrame\n{\n}\n', '- (void)endFrame\n{\n}\n\n- (void)addBackButton\n{\n  UIButton *backBtn = [UIButton buttonWithType:UIButtonTypeSystem];\n  UIImage *img = [UIImage systemImageNamed:@"chevron.backward"];\n  if (img) {\n    [backBtn setImage:img forState:UIControlStateNormal];\n  }\n  else {\n    [backBtn setTitle:@"Back" forState:UIControlStateNormal];\n  }\n  backBtn.tintColor = [UIColor whiteColor];\n  backBtn.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.45];\n  backBtn.layer.cornerRadius = 20.0;\n  backBtn.layer.masksToBounds = YES;\n  backBtn.translatesAutoresizingMaskIntoConstraints = NO;\n  [backBtn addTarget:self\n              action:@selector(handleBackButton)\n    forControlEvents:UIControlEventTouchUpInside];\n  [self addSubview:backBtn];\n  [NSLayoutConstraint activateConstraints:@[\n    [backBtn.leadingAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.leadingAnchor\n                                          constant:12.0],\n    [backBtn.topAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.topAnchor constant:12.0],\n    [backBtn.widthAnchor constraintEqualToConstant:40.0],\n    [backBtn.heightAnchor constraintEqualToConstant:40.0]\n  ]];\n}\n\n- (void)handleBackButton\n{\n  if (system && window) {\n    system->pushEvent(new GHOST_Event(\n        GHOST_GetMilliSeconds((GHOST_SystemHandle)system), GHOST_kEventWindowClose, window));\n  }\n}\n', 'win-backbtn-impl'),
  ('  [ghost_rootWindow registerGestureRecognizers];\n', '  [ghost_rootWindow registerGestureRecognizers];\n\n  /* build-18: secondary windows (render result, preferences, etc.) have no OS title bar\n   * on iOS; add an Apple-style back button (top-left) that closes this window and returns\n   * to the parent. The main window has no parent and gets none. */\n  if (parent_window != nullptr) {\n    [ghost_rootWindow addBackButton];\n  }\n', 'win-backbtn-ctor'),
])
print("BUILD-18 (magic-keyboard save-as + cursor offset + finger pinch + editor routing + window back button) APPLIED OK")
